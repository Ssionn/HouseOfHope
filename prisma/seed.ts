...
async function permissionSeed() {
  ...
  const permissionsToAssignToAdmin = await prisma.permission.findMany({
    where: {
      name: {
        in: ['isAdmin', 'edit_team_list', 'delete_team']
      }
    }
  });

  const rolePermissionsForTeamleader = permissionsToAssignToTeamleader.map((permission) => ({
    roleId: teamleaderRole.id,
    permissionId: permission.id
  }));

  const rolePermissionsForAdmin = permissionsToAssignToAdmin.map((permission) => ({
    roleId: teamleaderRole.id,
    permissionId: permission.id
  }));
  ...
  const updatedTeamLeaderRole = await prisma.role.update({
    ...
    data: rolePermissionsForTeamleader
  });

  const updatedAdminRole = await prisma.role.update({
    where: {id: adminRole.id},
    data: rolePermissionsForAdmin
  });
